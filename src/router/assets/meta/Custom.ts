import type { IMeta } from '@/models';

export const metaCustom: IMeta = {
  title: 'Wavelovers — Gamepad Custom Pattern',
  metaTags: [
    {
      name: 'keywords',
      content:
        'Wavelovers, Wave Lovers, Wavemaster, Wave Master, Vibration Master, Vibration, Gamepad, Gamepad Vibration, Gamepad Massager, Vibrate Gamepad, Mobile Vibration, Phone Vibration, Gamepad Tester, Phone Vibration Tester, Gamepad Vibration Tester, Vibration Tester, Massager, Vibrator, Satisfyer, Womanizer, Relax, Геймпад, Джойстик, Вибратор, Вибромассажер, Вибро, Вибромассажёр из геймпада, Тестер вибрации геймпада, Тестер вибрации телефона',
    },
    {
      name: 'description',
      content:
        'Wavelovers gamepad custom pattern mode. Wavelovers is a vibration test tool for gamepads and mobile devices that can also turn them into vibration massagers. It has free vibration templates, template editor and manual control mode.',
    },
    {
      name: 'og:title',
      content: 'Wavelovers — Gamepad Custom Pattern',
    },
    {
      name: 'og:description',
      content:
        'Wavelovers gamepad custom pattern mode. Wavelovers is a vibration test tool for gamepads and mobile devices that can also turn them into vibration massagers. It has free vibration templates, template editor and manual control mode.',
    },
    {
      name: 'og:url',
      content: 'https://wavelovers.ru/custom',
    },
    {
      name: 'twitter:title',
      content: 'Wavelovers — Gamepad Custom Pattern',
    },
    {
      name: 'twitter:description',
      content:
        'Wavelovers gamepad custom pattern mode. Wavelovers is a vibration test tool for gamepads and mobile devices that can also turn them into vibration massagers. It has free vibration templates, template editor and manual control mode.',
    },
  ],
  linkTags: [
    {
      rel: 'canonical',
      href: 'https://wavelovers.ru/custom',
    },
  ],
};

export default metaCustom;
